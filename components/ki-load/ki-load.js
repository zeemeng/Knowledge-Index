...
export class KiLoad extends Component {
  ...
  async connectedCallback() {
    ...
    const retry = this.getAttribute("retry") || 3;

    if (!type || !path)
      return console.error(
        "Attributes 'type' and 'path' must both be set with appropriate values before the 'ki-load' element is connected to the DOM."
      );
